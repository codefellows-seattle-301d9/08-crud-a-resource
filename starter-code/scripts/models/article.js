(function(module) {
  ...
  Article.createTable = function() {
    var inputString = 'CREATE TABLE IF NOT EXISTS articles_table (id INTEGER PRIMARY KEY, title VARCHAR, category VARCHAR, author VARCHAR, authorUrl VARCHAR, publishedOn DATE, body TEXT);';
      ...
      [
        {
          ...
          'sql': 'INSERT INTO articles_table (title, category, author, authorUrl, publishedOn, body) VALUES (?, ?, ?, ?, ?, ?);', // <----- complete our SQL command here, inside the quotes.
  ...
  Article.prototype.deleteRecord = function() {
    webDB.execute(
      [
        {
          'sql': 'DELETE FROM articles_table WHERE rowid = ?;', 
          'data': [this.id]
        }
      ]
    );
